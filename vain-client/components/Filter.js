    ...
    return (
        <div className="w-3/12">
			...
			<div className="w-full bg-white border-slate-300 border rounded-md drop-shadow-md p-5 mt-5">
				...
				{
					subjects.map((subject, index) => (<p key={index}><input type="checkbox" onChange={handleSubjectChange} checked={subjectsChecked.includes(subject.subjectdescription)} value={subject.subjectdescription} className="mr-2"/>{subject.subjectdescription}</p>))
				}
				<h1 className="font-bold text-xl mt-5">Types</h1>
				{
					types.map((type, index) => (<p key={index}><input type="checkbox" onChange={handleTypeChange} checked={typesChecked.includes(type.typedescription)} value={type.typedescription} className="mr-2"/>{type.typedescription}</p>))
				}
